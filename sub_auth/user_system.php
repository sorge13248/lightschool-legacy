<?php
  ...
  function lightschool_get_user($Username_function, $get_type){
    ...
    }elseif($get_type == 'id_to_username' or $get_type == 'id_to_complete'){
	...
	if($row['profile_image'] == 'default' or $row['profile_image'] == ''){
	  ...
	  $USER_FOUND_image = 'https://MAIN_HTTP_ADDRESS/my/'.$row['profile_image'];
	}
	...
	}elseif($get_type == 'complete' or $get_type == 'id_to_complete'){
	  return $USER_FOUND_name.' '.$USER_FOUND_surname;
	}elseif($get_type == 'completeinv'){
	  return $USER_FOUND_surname.' '.$USER_FOUND_name;
  ...
  function lightschool_get_ip($Username_function, $get_ip, $get_type){
	...
	while($GENERAL_row = $rs->fetch_assoc()){
	  if($get_type == 'id'){
	    ...
	    if($return == ''){
	      $return = "<span style='font-size: 10pt; color: gray'>User Agent non disponibile</span>";
	    }
	  }
  ...
  function lightschool_get_school_details($Username_function, $School_function, $get_type){
    ...
    if($num == 0){
      ...
      while($row = $rs->fetch_assoc()){
	if($get_type == 'periodo'){
	  $first_quad = $row['first_quad'];
	  $second_quad = $row['second_quad'];
