...
<script src="<?= $MY_MAIN_DIRECTORY ?>/js/jquery.cookie.js" type="text/javascript"></script>
...
<script type="text/javascript">
  $(document).ready(function(){
    <?php if($actually_page == 'settings'){ ?>
      ...
      $('#show_email').val('<?= $USER_visible_email ?>');
      $('#show_school').val('<?= $USER_visible_school ?>');
    <?php } ?>
    ...
    $("command").each(function(){
      ...
      if(!title){
	$(this).attr("title", "<?= $TRAD_keyboard_pressure ?> " + key);
      }
      if(!onclick){
	$(this).attr("onclick", "alert('<?= $TRAD_keyboard_pressure ?> " + key + "')");
      }
    });
  });
  ...
  $(document).keyup(function(e){
    if (e.keyCode == 27) {
      if($('.dialog').is(':visible') || $('.details_dialog').is(':visible')){
	closeDialog();
	closeDetailedDialog();
      }else{
...
<?php if($USER_first_login == 'y'){ ?>
  <link href="//cdnjs.cloudflare.com/ajax/libs/bootstrap-tour/0.10.1/css/bootstrap-tour-standalone.min.css" rel="stylesheet">
  <script src="//cdnjs.cloudflare.com/ajax/libs/bootstrap-tour/0.10.1/js/bootstrap-tour-standalone.min.js" type="text/javascript"></script>
  <script type="text/javascript">
    // Instance the tour
    var tour = new Tour({
      template: "<div class='popover tour'><div class='arrow'></div><h3 class='popover-title'></h3><div class='popover-content' style='font-size: 10pt'></div><div class='popover-navigation'><span class='link' data-role='prev'><?= $TRAD_back ?></span><span class='link' data-role='next' style='display: inline-block; margin-left: 15px; float: right'><?= $TRAD_forward ?></span></div></div>",
      backdrop: true,
      storage: false,
      steps: [
      {
	element: ".main_menu_button",
	placement: "right",
	title: "<?= $TRAD_tour_1 ?>",
	content: "<?= $TRAD_tour_1_descr ?>"
      },
      {
	element: ".settings_app",
	placement: "top",
	title: "<?= $TRAD_tour_2 ?>",
	content: "<?= $TRAD_tour_2_descr ?>"
      },
      {
	element: ".notification_app",
	placement: "left",
	title: "<?= $TRAD_tour_3 ?>",
	content: "<?= $TRAD_tour_3_descr ?>"
      },
      {
	orphan: true,
	title: "<?= $TRAD_tour_4 ?>",
	content: "<?= $TRAD_tour_4_descr ?>"
      },
      {
	element: "#q",
	placement: "bottom",
	title: "<?= $TRAD_tour_5 ?>",
	content: "<?= $TRAD_tour_5_descr ?>"
      },
      {
	element: ".help_tour",
	placement: "bottom",
	title: "<?= $TRAD_tour_6 ?>",
	content: "<?= $TRAD_tour_6_descr ?>"
      },
      {
	orphan: true,
	title: "<?= $TRAD_tour_7 ?>",
	content: "<?= $TRAD_tour_7_descr ?>"
      }
    ]});
...
<?php if($actually_page != 'register' and $actually_page != 'create_test' and $actually_page != 'settings' and $actually_page != 'trash' and $actually_page != 'search' and $actually_page != 'people'){ ?>
  <script type="text/javascript">
...
<style type="text/css">
  ...
  *{
  ...
  .header .title{
    font-size: 20pt;         
    <?php if($USER_taskbar_position == 'bottom' or $USER_taskbar_position == 'right'){ ?>
  ...
  .normal_taskbar{
    <?php if($USER_taskbar_position == 'bottom'){ ?>
      height: 40px;
    <?php }elseif($USER_taskbar_position == 'left' or $USER_taskbar_position == 'right'){ ?>
      height: calc(100% + 20px);
      width: 40px; 
    <?php } ?>
  }
  ...
  input.Username[type=text]{
    background: white url("<?= $IMAGES_MAIN_DIRECTORY ?>/<?= $USER_icon_set_black ?>/profile_male.png");
    padding-left: 30px !important;
    background-size: 16px 16px;  /* dimensioni sfondo */
    background-repeat: no-repeat;  /* immagine ripetuta solo una volta */
    background-position: 7px 7px;  /* spaziatura da sinistra e alto */
    vertical-align: middle; /* testo al centro verticalmente */
  }
  #Password{
  ...
  */
  ...
  .icon_files:hover, .selected, .selected2, .selected_file{
    background-color: rgba(<?= $r_lighter2 ?>, <?= $g_lighter2 ?>, <?= $b_lighter2 ?>, 0.50);
    border: 1px solid <?= $USER_accent ?> !important;
  }
  .icon_files span.title_files, .icon_files span.subtitle{
  ...
  hr.left{
    border: 0;
    height: 1px;
    background-image: -webkit-linear-gradient(left, rgba(0,0,0,1), rgba(255,255,255,0.50), rgba(0,0,0,0));
    background-image:    -moz-linear-gradient(left, rgba(0,0,0,1), rgba(255,255,255,0.50), rgba(0,0,0,0));
    background-image:     -ms-linear-gradient(left, rgba(0,0,0,1), rgba(255,255,255,0.50), rgba(0,0,0,0));
    background-image:      -o-linear-gradient(left, rgba(0,0,0,1), rgba(255,255,255,0.50), rgba(0,0,0,0));
  }
  ...
  /* images for diary */
  .diary_valentine{
    background-image: url("<?= $IMAGES_MAIN_DIRECTORY ?>/diary/valentine.png");
    background-size: 46px 52px;  /* dimensioni sfondo */
    background-position: right 10px bottom 10px;  /* spaziatura da sinistra e alto */
  }
  .diary_cat{
    background-image: url("<?= $IMAGES_MAIN_DIRECTORY ?>/diary/cat.png");
    background-size: 46px 52px;  /* dimensioni sfondo */
    background-position: right 10px bottom 10px;  /* spaziatura da sinistra e alto */
  }
  .diary_women{
    background-image: url("<?= $IMAGES_MAIN_DIRECTORY ?>/diary/women.png");
    background-size: 60px 60px;  /* dimensioni sfondo */
    background-position: right 0px bottom 0px;  /* spaziatura da sinistra e alto */
  }
  .diary_dad{
    background-image: url("<?= $IMAGES_MAIN_DIRECTORY ?>/diary/dad.png");
    background-size: 60px 80px;  /* dimensioni sfondo */
    background-position: right 0px bottom 0px;  /* spaziatura da sinistra e alto */
  }
  .diary_fool{
    background-image: url("<?= $IMAGES_MAIN_DIRECTORY ?>/diary/fool.png");
    background-size: 80px 60px;  /* dimensioni sfondo */
    background-position: right 10px bottom 10px;  /* spaziatura da sinistra e alto */
  }
  .diary_easter{
  ...
  .diary_earth{
    background-image: url("<?= $IMAGES_MAIN_DIRECTORY ?>/diary/earth.png");
    background-size: 60px 60px;  /* dimensioni sfondo */
    background-position: right 10px bottom 10px;  /* spaziatura da sinistra e alto */
  }
  .diary_antifascismo{
  ...
  .diary_work{
    background-image: url("<?= $IMAGES_MAIN_DIRECTORY ?>/diary/work.png");
    background-size: 60px 60px;  /* dimensioni sfondo */
    background-position: right 10px bottom 10px;  /* spaziatura da sinistra e alto */
  }
  .diary_mum{
    background-image: url("<?= $IMAGES_MAIN_DIRECTORY ?>/diary/mum.png");
    background-size: 50px 60px;  /* dimensioni sfondo */
    background-position: right 10px bottom 10px;  /* spaziatura da sinistra e alto */
  }
  .diary_tobacco{
    background-image: url("<?= $IMAGES_MAIN_DIRECTORY ?>/diary/tobacco.png");
    background-size: 40px 30px;  /* dimensioni sfondo */
    background-position: right 10px bottom 10px;  /* spaziatura da sinistra e alto */
  }
  .diary_mandela{
    background-image: url("<?= $IMAGES_MAIN_DIRECTORY ?>/diary/mandela.png");
    background-size: 62px 62px;  /* dimensioni sfondo */
    background-position: right 0px bottom 0px;  /* spaziatura da sinistra e alto */
  }
  ...
  .tab{
    ...
    <?php }else{ ?>
      ...
      cursor: default;
      min-width: 200px;
      <?php if($USER_taskbar_position == 'left'){ ?>
	width: calc(100% - 25px);
      <?php }elseif($USER_taskbar_position == 'right'){ ?>
	width: calc(100% - 60px);
      <?php }else{ ?>
  ...
  <?php if($USER_taskbar_position == 'bottom'){ ?>
    .taskbar_tray span, .taskbar_tray a{
      display: inline-block;
      float: right;
    }
    .taskbar_tray .app{
  ...
  .notification_center{
    ...
    z-index: 9999999;
    cursor: default;
    -webkit-box-shadow: -5px 0px 14px 0px rgba(50, 50, 50, 1);
    -moz-box-shadow:    -5px 0px 14px 0px rgba(50, 50, 50, 1);
    box-shadow:         -5px 0px 14px 0px rgba(50, 50, 50, 1);
    overflow-y: auto;
  }
  ...
  .notification_title, .notification_descr, .notification_actions{
    padding-top: 5px;
    padding-bottom: 5px;
  }
  .notification_title{
  ...
  .hidden_settings > input[type=checkbox] > label{
  ...
  .model:hover, .profile_frame:hover{
  ...
  .register_content .tabs, .register_content .tab{
    border: none;
  }
  .register_content .tab_content{
    padding: 10px;
  }
...
<div class="context context_app" id="contextapp"></div>
...
<div class="overlay" id="notificationcenter_overlay" onclick="closenotificationcenter()" oncontextmenu="return false"></div>
...
<script type="text/javascript">
  ...
  function closenotificationcenter(){
    $("#notificationcenter_overlay").fadeOut(300);
    $(".notification_center").hide("slide", { direction: "right" }, 300);
  }
</script>