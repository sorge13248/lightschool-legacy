...
function lightschool_send_email($type, $address, $name, $surname, $username, $id, $sex, $accent, $fore_color, $field6, $field7, $field8, $lang){
  ...
  if($type == 'register'){
    $link1 = "https://my.lightschool.it/activate?code=$field6";
    ...
    $link1 = "https://my.lightschool.it/recover?code=$field6";
  ...
  try{
    $mail->Host       = "mail.lightschool.it"; // SMTP server
    // $mail->SMTPDebug  = 2;                     // enables SMTP debug information (for testing)
    $mail->SMTPAuth   = true;                  // enable SMTP authentication
    $mail->Host       = "mail.lightschool.it"; // sets the SMTP server
    $mail->Port       = 25;                    // set the SMTP port for the GMAIL server
    $mail->Username   = "noreply@lightschool.it"; // SMTP account username
    $mail->Password   = ";        // SMTP account password
    $mail->AddAddress("$address", "$AddAddressName");
    $mail->SetFrom('noreply@lightschool.it', "$SetFromName");
